...
/**
 * `usage_logs` — daily counter for the AI watermark detector.
 *
 * One row per identity (user_id when logged-in, ip_hash when anonymous) per
 * UTC date. Used by `src/lib/usage.ts` to enforce the daily quota
 * (PRD R-0401 / R-0403 / R-0601).
 *
 * `user_id` is empty string '' for anonymous rows so the unique-by-date
 * index works across both dialects (SQLite can't put UNIQUE on a NULL column).
 */
export const usageLogs = table(
  'usage_logs',
  {
    id: text('id').primaryKey(),
  ...
  (table) => [
    index('idx_usage_logs_user_date').on(table.userId, table.date),
    index('idx_usage_logs_ip_date').on(table.ipHash, table.date),
  ]
);
